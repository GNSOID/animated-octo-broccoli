...
给手机号和密码的标签添加上边框和阴影
ChatGPT
<style> block. Here's an updated version of the code that adds borders and shadows:

 html Copy code <template><div class="box">< !-- 头部 --><header class="header">您好</header>< !-- 表单 --><van-form @submit="onSubmit"><van-cell-group inset><van-field v-model="username"
 placeholder="手机号"
 :rules="rules.username"
 class="input-field"
 ...
 :rules="rules.password"
 class="input-field"
 /></van-cell-group><div class="btn1"><van-button round block type="success" native-type="submit">登录 </van-button><van-button class="btn2"
 round block type="warning"
 @click="$router.push('/register/step1')"

 >注册 </van-button></div></van-form></div></template><style scoped>.header {
   width: 100%;
   height: 0.44rem;
   line-height: 0.44rem;
   color: black;
